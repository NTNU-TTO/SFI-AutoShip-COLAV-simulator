...
class Simulator:
    ...
    def run(self, horizon: float, dt: float) -> None:
        ...
        Args:
            horizon (float): Time horizon for the simulation
...
def run_scenario_simulation(ships, time, timestep):
    ...
    for i in range(1, len(ships) + 1):
        ...
        try:
            data[f"Ship{i}"][4] = ships[i - 1].wp
        except:
            print(f"Ship{i} has no waypoints")
...
def init_scenario(is_new_scenario, scenario_file):
    """
    Returns initialized and configured ships, and their route plans
    is_new_scenario=False: creates a new scenario based on scenario_num and saves
        the poses and plans to scenario_file
    is_new_scenario=True: Loads poses and plans from scenario_file
