"""
    ...
    Grounding hazard avoidance implemented by Stefan Lapadatu
"""
...
class SBMPCParams:
    ...
    P_ca_: np.array = field(default_factory=lambda: np.array([0.25, 0.5, 1.0]))  # control behaviors - speed factor (foreløpig fjernet mulighet for rygging)
...
class SBMPC:
    def __init__(self, config: Optional[SBMPCParams] = None) -> None:
        ...
        self.T_GH_ = 60.0                               # seconds to look ahead for grounding hazards
        self.t_last_hazard_check = time.time()-1000.0   # timestamp [s] of last check for distance to closest grounding hazard
        self.d_last_hazard_check = 0.0                  # distance to closest grounding hazard at last check

        self.cost_ = np.inf

        self.ownship = ShipModel(self.T_, self.DT_)
        self.relevant_hazards = []
    ...
    def get_optimal_ctrl_offset(
        ...
        # If being run the first time, set the relevant hazards based on the enc used
        if self.relevant_hazards != hazards:
            self.relevant_hazards = hazards
        ...
        SB_0, AH_0, HO_0 = np.zeros(len(do_list)), np.zeros(len(do_list)), np.zeros(len(do_list))
        ...
        for i in range(len(self._params.Chi_ca_)):
            for j in range(len(self._params.P_ca_)):
                ...
                # Check for grounding hazards only every x seconds or if the ship is close to a grounding hazard to save computation time.
                # FIXME magic numbers: draft depth, distance considered close for grounding, how often to check for grounding hazards (time and distance)
                if time.time() - self.t_last_hazard_check > 10.0 or self.d_last_hazard_check < 500.0:
                    # Notice that x and y naming convention is opposite in the function mapf.distance_to_enc_hazards
                    distance_to_grounding_hazard = mapf.distance_to_enc_hazards(self.ownship.y_[0], self.ownship.x_[0], mapf.find_minimum_depth(5.0, enc), enc, hazards)
                    self.d_last_hazard_check = distance_to_grounding_hazard
                    self.t_last_hazard_check = time.time()

                    if self.check_course_for_grounding(enc):
                        #print("Grounding hazard detected!")
                        continue

                cost_i = -1                        
                for k in range(n_obst):
                    cost_k = self.cost_func(self._params.P_ca_[j], self._params.Chi_ca_[i], self._params.Chi_ca_last_, obstacles[k], 
    ...
    def check_course_for_grounding(self, enc):
        """Checks if the current course leads to grounding. Uses two more points towards both diagonals
        of the ship as a buffer by checking two additional segments for grounding.

        Args:
            enc (ENC): Electronic Navigational Chart object
            
        returns:
            bool: True if current course leads to a grounding, False otherwise.
        """
        time_started = time.time()

        end_index = int(min(self.T_GH_/self.DT_, self.n_samp))
        p1 = np.array([self.ownship.x_[0], self.ownship.y_[0]])
        p2 = np.array([self.ownship.x_[end_index], self.ownship.y_[end_index]])
        
        # Use two diagonal points to check for collisions that aren't to the center of the ship
        p1_diag_1  = np.array([self.ownship.x_[0] - 2*self.ownship.w, self.ownship.y_[0] - 2*self.ownship.l])
        p1_diag_2  = np.array([self.ownship.x_[0] + 2*self.ownship.w, self.ownship.y_[0] + 2*self.ownship.l])
        
        collision_straight = mapf.check_if_segment_crosses_grounding_hazards(enc, p1, p2, hazards=self.relevant_hazards)
        collision_diag_1  = mapf.check_if_segment_crosses_grounding_hazards(enc, p1_diag_1, p2, hazards=self.relevant_hazards)
        collision_diag_2  = mapf.check_if_segment_crosses_grounding_hazards(enc, p1_diag_2, p2, hazards=self.relevant_hazards)
        collision = collision_straight or collision_diag_1 or collision_diag_2

        #print("Grounding hazard check took: ", time.time() - time_started)
