...
class SBMPCParameterObservation(ObservationType):
    """Observation containing the SB-MPC parameters. Used together with the SB-MPC action defined in action.py"""
    ...
    def __init__(
        ...
        super().__init__(env)
        assert self.env.ownship is not None, "Ownship is not defined"
        assert self.env.action_type.sbmpc_obj is not None, "SBMPC not defined in action type"
        self.name = "SBMPCParameterObservation"
        self.sbmpc_param_ranges, self.sbmpc_param_incr_ranges = (
            self.env.action_type.sbmpc_params.get_tuneable_param_info()
        )
        self.sbmpc_tuneable_param_list = env.action_type.sbmpc_tuneable_param_list
    ...
    def space(self) -> gym.spaces.Space:
        return gym.spaces.Box(low=-1.0, high=1.0, shape=(self.n_tuneable_params,), dtype=np.float32)

    def normalize(self, obs: Observation) -> Observation:
        return sbmpc_hf.normalize_sbmpc_param(
            param_obs=obs,
    ...
    def unnormalize(self, obs: Observation) -> Observation:
        return sbmpc_hf.unnormalize_sbmpc_param(
            param_obs=obs,
            param_list=self.sbmpc_tuneable_param_list,
            parameter_ranges=self.sbmpc_param_ranges,
        )

    def observe(self) -> Observation:
        assert hasattr(self.env.action_type, "sbmpc_obj"), "Must have SB-MPC in the action type for this observation to work!"
        if self.env.time < 0.0001:
            obs = self.env.action_type.current_param_values_init
        else:
            obs = self.env.action_type.sbmpc_params.get_tuneable_param_values(param_list=self.sbmpc_tuneable_param_list)
        normalized_obs = self.normalize(obs)
        return normalized_obs
    
class DistanceBearingToHazardObservation(ObservationType):
    """Observation containing distances and bearings to points that are part of the closest grounding hazard."""

    def __init__(self, env: "COLAVEnvironment") -> None:
        super().__init__(env)
        self.name = "DistanceBearingToHazardObservation"
        self.observation_size = 2  # [distance, bearing]
        self.closest_N = 10 # Number of points in closest hazard to consider
    ...
    def space(self) -> gym.spaces.Space:
        return gym.spaces.Box(low=-1.0, high=1.0, shape=(self.closest_N, self.observation_size), dtype=np.float32)

    def define_observation_ranges(self) -> None:
        """Define the valid ranges for distance and bearing."""
        self.observation_range = {
            "distance": (0.0, 500.0),
            "bearing": (-np.pi, np.pi),
        }
    ...
    def normalize(self, obs: Observation) -> Observation:
        ...
        for idx in range(obs.shape[0]):
            norm_obs[idx, 0] = mf.linear_map(obs[idx, 0], self.observation_range["distance"], (-1.0, 1.0))
            norm_obs[idx, 1] = mf.linear_map(obs[idx, 1], self.observation_range["bearing"], (-1.0, 1.0))
    ...
    def unnormalize(self, obs: Observation) -> Observation:
        ...
        for idx in range(obs.shape[0]):
            unnorm_obs[idx, 0] = mf.linear_map(obs[idx, 0], (-1.0, 1.0), self.observation_range["distance"])
            unnorm_obs[idx, 1] = mf.linear_map(obs[idx, 1], (-1.0, 1.0), self.observation_range["bearing"])
        return unnorm_obs
    
    def observe(self) -> Observation:
        """Return distance and bearing from ownship to N closest points in the closest hazard."""
        ownship_pos = self.env.ownship.csog_state[:2]
        ownship_point = shapely.Point(ownship_pos[1], ownship_pos[0])  # (x, y) = (east, north)

        closest_hazard = min(
            self.grounding_hazards,
            key=lambda hazard: ownship_point.distance(hazard)
        )

        hazard_points = list(closest_hazard.exterior.coords if hasattr(closest_hazard, 'exterior')
                        else closest_hazard.coords)

        distances_and_bearings = []
        for point in hazard_points:
            point = shapely.Point(point)
            dist = ownship_point.distance(point)
            bearing = np.arctan2(
                point.y - ownship_pos[0],  # delta north
                point.x - ownship_pos[1]   # delta east
            )
            rel_bearing = mf.wrap_angle_diff_to_pmpi(bearing, self.env.ownship.course)
            distances_and_bearings.append((dist, rel_bearing))

        # Filter for points in front of the ownship
        filtered_distances_and_bearings = [
            (dist, bearing) for dist, bearing in distances_and_bearings
            if np.cos(bearing) > 0
        ]

        # Sort by distance and select the N closest points
        filtered_distances_and_bearings.sort(key=lambda x: x[0])
        selected = filtered_distances_and_bearings[:self.closest_N]

        # Pad if fewer than closest_N points
        while len(selected) < self.closest_N:
            selected.append((self.observation_range["distance"][1], self.observation_range["bearing"][1]))

        obs = np.array(selected, dtype=np.float32)  # Shape: (N, 2)
        norm_obs = self.normalize(obs)
        epsilon = 10e-6
        return np.clip(norm_obs, -1.0 + epsilon, 1.0 - epsilon)
...
def observation_factory(
    ...
    elif "sbmpc_parameter_observation" in observation_type:
        return SBMPCParameterObservation(env, **kwargs)
    elif "distance_bearing_to_grounding_hazards_observation" in observation_type:
        return DistanceBearingToHazardObservation(env, **kwargs)
